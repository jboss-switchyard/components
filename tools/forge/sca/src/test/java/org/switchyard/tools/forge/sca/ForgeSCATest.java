...
/**
 * Test for {@link RemoteFacet}.
 */
public class ForgeSCATest extends GenericTestForge {
    ...
    private static final String SCA_SUCCESS_MSG = "***SUCCESS*** Installed [switchyard.sca] successfully";
    
    private static final String SCA_BINDING_MSG = "Added binding.sca to service " + BEAN_SERVICE;
    
    private static final String SCA_REFERENCE_MSG = "Added binding.sca to reference " + BEAN_SERVICE_REFERENCEABLE;
    ...
    public static JavaArchive getDeployment() {
        ...
        archive.addPackages(true, SCAFacet.class.getPackage());
    ...
    public void test() throws Exception {
        try {
            //remote-binding bind-service
            testBindService();
            //remote-binding bind-reference
            testBindReference();
        } catch (Exception e) {
    ...
    private void testBindService() throws Exception {
        resetOutputStream();
        getShell().execute("project install-facet switchyard.sca");
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(SCA_SUCCESS_MSG));
        resetOutputStream();
        ...
        getShell().execute("sca-binding bind-service --serviceName " + BEAN_SERVICE);
        
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(SCA_BINDING_MSG));
    }
    ...
    private void testBindReference() throws Exception {
        ...
        getShell().execute("sca-binding bind-reference --referenceName " + BEAN_SERVICE_REFERENCEABLE);
        getShell().execute("switchyard show-config");
        
        System.out.println(getOutput());
        Assert.assertTrue(getOutput().contains(SCA_REFERENCE_MSG));
    }
