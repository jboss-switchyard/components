...
public class ForgeCamelTest extends AbstractShellTest {
    ...
    private static final String switchyardSuccessMsg = "SwitchYard version " + switchyardVersion;
    ...
    private void testBindReference() throws Exception {
        ...
        File to = new File(getShell().getCurrentDirectory().getUnderlyingResourceObject().getAbsolutePath() + 
    ...
    private void prepareSwitchyardForge() throws IOException {
    	try {
			...
			Assert.assertTrue(outputStream.toString().contains(switchyardSuccessMsg));
			System.out.println(outputStream);
