...
public class ClojureModelBuilderTest {
    ...
    public void injectExchange() throws Exception {
        final ClojureComponentImplementationModel implModel = new ClojureModelBuilder().injectExchange(true).build();
    ...
    public void inlineScript() throws Exception {
        ...
        final ClojureComponentImplementationModel implModel = new ClojureModelBuilder().inlineScript(scriptFile).build();
    ...
    public void externalScript() throws Exception {
        ...
        final ClojureComponentImplementationModel implModel = new ClojureModelBuilder().externalScript(scriptFile).build();
        assertThat(implModel.getScriptFile(), is(equalTo(scriptFile)));
    }
    
    @Test
    public void should_throw_exception_if_both_inlineScript_and_externalScript_have_been_set() throws Exception {
        thrown.expect(ClojureBuilderException.class);
        thrown.expectMessage("Only one of 'inlineScript' and 'externalScript' can be set, not both!");
        new ClojureModelBuilder().externalScript("bla").inlineScript("bla").build();
    }
