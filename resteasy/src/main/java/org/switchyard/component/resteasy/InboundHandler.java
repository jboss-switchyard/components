...
import org.switchyard.component.resteasy.composer.ContentRESTEasyMessage;
...
public class InboundHandler extends BaseServiceHandler {
    ...
    public Object invoke(final String operationName, final Object input, final boolean oneWay) {
        ContentRESTEasyMessage restMessageSource = new ContentRESTEasyMessage(input);
        Object output = null;
        try {
            SynchronousInOutHandler inOutHandler = new SynchronousInOutHandler();
            Exchange exchange = _service.createExchange(operationName, inOutHandler);
            Message message = _messageComposer.compose(restMessageSource, exchange, true);
            if (oneWay) {
                exchange.send(message);
            } else {
                exchange.send(message);
                exchange = inOutHandler.waitForOut();
                ContentRESTEasyMessage restMessageTarget = (ContentRESTEasyMessage)_messageComposer.decompose(exchange, new ContentRESTEasyMessage());
                output = restMessageTarget.getContent();
            }
        } catch (Exception e) {
